...
void query_sum(int node, int qstart, int qend, int &ans) {
    if (segTree[node].left > qend || segTree[node].right < qstart)  {
        return ;
    }
    if (segTree[node].left == segTree[node].right &&  qstart <= segTree[node].left && qend >= segTree[node].right) {
        ans += segTree[node].val;
        return;
    }

    query_sum(node * 2, qstart, qend, ans);
    query_sum(node * 2 + 1, qstart, qend, ans);
}
...
void print() {
    for (int i = 1; i <= 3 * n; i++) {
        printf("%d:  %d    left: %d        right: %d\n", i, segTree[i].val, segTree[i].left, segTree[i].right);
    }
    printf("\n");
}
